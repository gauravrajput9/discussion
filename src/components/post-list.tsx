...
type DisplayPostsListProps = {
  posts: PostType[];
  slug?: string; // <-- keep this optional
};

const DisplayPostsList = ({ posts }: DisplayPostsListProps) => {
      ...
      {posts.length > 0 ? (
        <ul className="space-y-6">
          {posts.map((post) => (
            <li
              key={post.id}
              className="bg-gradient-to-br from-purple-600 to-indigo-600 text-white border border-purple-300 rounded-xl p-6 shadow-md hover:shadow-xl transition-all duration-300"
            >
              {/* Title */}
